class BattleshipRenderer {
    ...
    async updatePlayerInfo() {
        ...
        if (myPlayer) {
            ...
            if (this.battleshipGame.player1Name) {
                this.battleshipGame.player1Name.textContent = myPlayer.username;
            }
            if (this.battleshipGame.ownBoardTitle) {
                this.battleshipGame.ownBoardTitle.textContent = `${myPlayer.username}'s Fleet`;
            }
        ...
        if (opponentPlayer) {
            ...
            if (this.battleshipGame.player2Name) {
                this.battleshipGame.player2Name.textContent = hasTwoPlayers ? opponentPlayer.username : 'Waiting...';
            }
            if (this.battleshipGame.opponentBoardTitle) {
                this.battleshipGame.opponentBoardTitle.textContent = `${opponentPlayer.username}'s Fleet`;
            }
            ...
            if (this.battleshipGame.opponentBoardTitle) {
                // Keep as "Waiting..." or use last known opponent if available
                const lastKnownOpponent = this.app.lastKnownPlayers?.find(p => p.username !== this.app.currentUser);
                if (lastKnownOpponent) {
                    this.battleshipGame.opponentBoardTitle.textContent = `${lastKnownOpponent.username}'s Fleet`;
                } else {
                    this.battleshipGame.opponentBoardTitle.textContent = "Waiting for opponent...";
                }
            }
            if (this.battleshipGame.player2Role) {
                this.battleshipGame.player2Role.textContent = '';
